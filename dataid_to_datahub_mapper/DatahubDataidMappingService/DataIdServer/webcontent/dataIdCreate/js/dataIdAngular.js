...
function genController($scope, $modal, $http) {
    ...
    $scope.addDataset2 = function(parent) {
        ...
        $scope.pushNewSet(parent, dataset);
        return dataset;
    };
    ...
    {
        var ttl = JSON.stringify(cleanDataId(dataid));
        var request = sendRequest("http://akswnc7.informatik.uni-leipzig.de:9997/dataid/publisher/validateid", "POST", ttl, true, function(e){
    ...
    $scope.$watchCollection('graph', function(newVal){
        ...
        alert(JSON.stringify(newVal[newVal.length-1]));
